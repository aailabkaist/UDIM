...
def train(test_envs, args, hparams, n_steps, checkpoint_freq, logger, writer, target_env=None):
    ...
    logger.info(f"given {n_envs} environments, we train {len(train_envs)} environments and test {len(set(test_envs))} environments.")
    ...
    for step in range(n_steps):
        ...
        inputs = {**batches, "step": step,"n_steps" : n_steps}
        ...
        if step % checkpoint_freq == 0 and step != 0:
            ...
            # if step % 1000 == 0:
            #     accuracies, summaries, perturb_lists = evaluator.evaluate(algorithm,perturb=True)
            #     with open(args.out_root / f'perturb_train{test_envs}_step{str(step)}','w') as fp:
            #         json.dump(perturb_lists,fp)
...
def warmup_train(test_envs, args, hparams, n_steps, checkpoint_freq, logger, writer, target_env=None):
    logger.info("")
    ...
    for step in range(n_steps):
        ...
        if step % checkpoint_freq == 0 or step == n_steps-1:
            ...
            warmup_val_acc = summaries['train_out']
            if warmup_best_val_acc < warmup_val_acc:
                warmup_best_val_acc = warmup_val_acc
                save_dict = {
                    "model_state_dict": algorithm.network.state_dict(),
                    "optimizer": algorithm.optimizer.state_dict(),
                }
                print('Model is saved, with acc', warmup_best_val_acc)
                save_checkpoint=True
                torch.save(save_dict, args.out_dir / 'eval_best.pt')
    ...
    # if no model_checkpoint, save last
...
def main_train(test_envs, args, hparams, start_steps, n_steps, checkpoint_freq, logger, writer, target_env=None):
    logger.info("")
    ...
    for step in range(start_steps,n_steps):
        ...
        inputs = {**batches, "step": step,"n_steps" : n_steps-start_steps}
        step_vals = algorithm.update(**inputs)
